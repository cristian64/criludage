        ...
        private void timerConsumirSolicitudes_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (consumidorSolicitudes.ConexionActiva())
                {
                    ...
                    if (xml != null)
                    {
                        String[] clavePublica = Program.InterfazRemota().ObtenerClavePublica(Configuracion.Default.usuario, Configuracion.Default.contrasena).Split(' ');
                        RSA enc = new RSA("0", clavePublica[0], clavePublica[1]);
